...
class _ShowCalScreenState extends State<ShowCalScreen> {
  ...
  int height = 167;
  int age = 22;
  int calsNow = 800; // แคลลอรี่ที่เหลือในวันนี้ ต้องมาจากหน้าเพิ่มอาหารในฐานข้อมูล
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('หน้าแสดงแคลลอรี่')),
      body: Padding(
        ...
        child: CircularPercentIndicator(
          ...
          footer: Column(
            ...
            children: [
              ...
              Text('แคลลอรี่คนไม่ออกกำลังกาย: '+calNone.toString()+' แคล',),
              Text('แคลลอรี่คนออกกำลังกาย 1-3 วัน: '+cal_1to3day.toString()+' แคล',),
              Text('แคลลอรี่คนออกกำลังกาย 4-5 วัน: '+cal_4to5day.toString()+' แคล',),
              Text('แคลลอรี่คนออกกำลังกาย 6-7 วัน: '+cal_6to7day.toString()+' แคล',),
              Text('แคลลอรี่คนออกกำลังกาย 2 ครั้งต่อวัน: '+cal_2perday.toString()+' แคล',),
              Text('โปรตีน: '+pro.toString()+' กรัม',),
              Text('คาร์โบไฮเดตร: '+carb.toString()+' กรัม',),
              Text('ไขมัน: '+fat.toString()+' กรัม',),
            ],
          ),
          center: Column(
            children: [
              ...
              Text(
                age.toString()+' year  '+height.toString()+' cm', // แสดงอายุกับส่วนสูง
                style: TextStyle(
                  color: Colors.blueAccent,
                  fontSize: 20.0,
                ),
  ...
  void calRemain(int calnow, int calnone) {
    ...
    */
    ...
    if(cirper>1) {
      setState(() {
        circlePercent = cirper-1.0; // ต้องไปจัดการว่าค่าแคลลอรี่ที่เหลือจะเอายังไง
      });
    } else {
      setState(() {
